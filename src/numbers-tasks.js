...
function toPrecision(number, precision) {
  ...
  if (decimalPart) {
    adjustedDecimalPart = decimalPart.padEnd(
      precision - integerPart.length,
      '0'
    );
  } else if (precision > integerPart.length) {
    adjustedDecimalPart = '0'.repeat(precision - integerPart.length);
  }
  return adjustedDecimalPart
    ? `${integerPart}.${adjustedDecimalPart}`
    : integerPart;
}
...
function roundToSmallestInteger(number) {
  ...
  return Math.ceil(Math.abs(number)) * -1;
}
